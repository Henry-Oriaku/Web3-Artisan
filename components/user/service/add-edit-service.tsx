...
export default function AddEditService({ onAdded, onCancel }: { onAdded: (service: Service) => void, onCancel: () => void }) {
    const [service, setService] = useState<Service>({ description: '', title: '', image: '', negotiable: false, price: 0, imageFile: null })

    function updateService(key: string, value: any) {
        if (key == 'imageFile') {
